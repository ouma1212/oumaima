...
query.prepare("INSERT INTO client (NUM_CLIENT, NOM, PRENOM, ADRESSE, DATE_NAISS) "
...
bool Client::modifier1(int idd)
{
    QSqlQuery query;
    QString res= QString::number(idd);
    query.prepare("UPDATE client SET NOM=:nom,PRENOM=:prenom,ADRESSE=:adresse,DATE_NAISS=:DATE_NAISS WHERE NUM_CLIENT=:id");
    query.bindValue(":id",res);
    query.bindValue(":nom",nom);
    query.bindValue(":prenom",prenom);
    query.bindValue(":adresse",adresse);
